...
\parte{Restricciones y dependencias}{CON-02 (TCP), CON-03 (sin WebSockets), REQ-03 (segundo factor), DEP-02 (proveedor del segundo factor).}
...
\textbf{Por qué es relevante.} Con TCP directo y sin WebSockets, el juego no hereda ningún cifrado de la plataforma: si el equipo no lo resuelve, todo viaja en claro. En ese caso, el segundo factor que exige REQ-03 no protege nada, porque el código se puede leer y el identificador de sesión se puede robar después de haberlo validado (Q-09 lo señala). Los usuarios son niños que juegan en redes de escuela, justo donde este ataque es más fácil, y el chat contiene datos personales de menores (CRN-05). La última medida hace visible la tensión con el desempeño: la protección cuesta tiempo en cada mensaje y no puede romper ESC-01.
